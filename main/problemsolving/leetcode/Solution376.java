...
public class Solution376 {
    ...
    public int wiggleMaxLength(int[] nums) {
        ...
        for (int i = 1; i < diff.length; i++) {
            // up -> down
            if (nowDiff < 0 && diff[i] > 0) {
                nowDiff = diff[i];
                answer++;
                // down -> up
            } else if (nowDiff > 0 && diff[i] < 0) {
                nowDiff = diff[i];
                answer++;
                // none -> up | down
            } else if (nowDiff == 0 && diff[i] != 0) {
